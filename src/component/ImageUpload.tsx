 ...
 const [textAvatars, settextAvatars] = useState<any[]>([])
  ...
  const saveTestFunction = async() =>{
    ...
    task.on('state_changed', snapshot =>{
        console.log(`${snapshot.bytesTransferred} transfere out of ${snapshot.totalBytes}`);
    ...
    <View style={[styles.container]}>
      ...
      <View style={styles.container_images}>
        <TouchableOpacity onPress={() => choose_photo()}>
          {image && <Image source={{uri: image}} style={styles.image_picker} />}
          {!image && (
              <Image
      ...
      <TouchableOpacity
        ...
        <Text style={{fontSize: 30, fontFamily: 'Inter', fontStyle: 'italic'}}>
          save
        </Text>
      </TouchableOpacity>
      ...
      { allAvatars ? (
          allAvatars.map((avatar: any, index:number) =>(
            <Image key={index} source={{ uri: avatar }} style={styles.image_returned} />))
              ):null
